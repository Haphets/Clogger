...
public class LogManager extends Thread{
    ...
    /** 日志文件列表 */
    private Map<String,LogFileProperties> logFileMap = new ConcurrentHashMap<String,LogFileProperties>();
    ...
    public void addLog(String logFileName,StringBuffer logMsg){
        //获得单个日志文件的信息
        LogFileProperties lfi = logFileMap.get(logFileName);
        if(lfi == null){
            synchronized(this){
                lfi = logFileMap.get(logFileName);
                if(lfi == null){
                    lfi = new LogFileProperties();
    ...
    private void flush(boolean bIsForce) throws IOException {
        ...
        while(iter.hasNext()){
            LogFileProperties lfi = logFileMap.get(iter.next());
     ...
     * @param lfi
     */
    private void createLogFile(LogFileProperties lfi){
        //当前系统日期
